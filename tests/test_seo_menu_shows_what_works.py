"""The SEO menu shows only what works today, and Articles is as dense as the inbox.
...
I can start using it to promote Ownbox content"*):
  (a) the SEO menu has only rows that work today. No empty Today, Search or AI rows;
...
Data sources and its nested menu are OSDev6's (#1572) and are checked by test_seo_sources.
And the owner, 2026-09-25: the list needs *"twice as much information on the page… Similar to the
inbox."*

Run: python tests/test_seo_menu_shows_what_works.py
"""
import os
import re
import sys
...
os.environ["AIOS_DB_PATH"] = os.path.join(_T, "seomenu.db")
...
# A BOX SHIPS ONLY ITS OWN MACHINES: a box without the SEO machine has nothing here to check.
try:
    from marketing.seo_machine import app as seo_app                  # noqa: E402
    from marketing.seo_machine import plan                            # noqa: E402
except ImportError:
    print("  --   the SEO machine is not on this box; nothing to check here")
    print("\nall good")
    raise SystemExit(0)
...
seo = next((s for s in shell.sections() if s.key == "seo"), None)
labels = [i.label for i in seo.items] if seo else []
ok("SEO opens on Articles and ends on Settings",
...
for item in (seo.items if seo else ()):
...
for i, (t, q) in enumerate((("Microneedling aftercare", "What should I do after microneedling?"),
    ...
    plan.add(t, q)
page = owner.get("/seo/topics").get_data(as_text=True)
main = page.split("</nav>", 1)[-1]
...
_real = seo_app.writer_installed, seo_app.missing
seo_app.writer_installed, seo_app.missing = (lambda: True), (lambda: [])
try:
    page = owner.get("/seo/topics").get_data(as_text=True)
    ...
    seo_app.writer_installed, seo_app.missing = _real
