...
# TWO SINCE 2026-09-22, AND THAT IS A SPLIT RATHER THAN A DELETION. `_setup_source` answers "what
# does the MACHINE's set-up screen show", and the owner ruled that day that the AI account, the
# phone and the AI coworkers are the box's settings, finished in core's own drawer: "Step four
# and five should not be in this wizard any longer."
#
# THE THREE THAT LEFT ARE ASSERTED STILL PRESENT IN THE CONTRACT, two lines down, because the
# failure this baseline is really guarding against — a step silently disappearing — looks exactly
# like a correct split if you only count what the screen renders.
ok("the machine's screen shows the machine's credentials",
...
ok("A NEW SEAM STEP ADDS ITSELF AND DELETES NOTHING — the whole bug, in one assertion",
   after == ["email", "zernio", "relay"], str(after))
...
ok("...and the buyer's own order is untouched", after[:len(base)] == base, str(after))
...
ok("...IN PLACE, not jumped to the end by its order — the owner set this screen's order",
   keys == ["email", "zernio", "relay"], str(keys))
...
ok("A SEAM THAT RAISES NEVER TAKES THE SET-UP PAGE DOWN — the old contract renders alone",
   survived == ["email", "zernio"], str(survived))
