"""The AEO machine's screens: Topics (what to write), Data sources (Sanity and Airtable) and Settings
...
`aeo`, and read back through `settings.get()` — the one accessor the publisher and the writer read.
...
nowhere else; AEO Settings no longer carries them (owner, 2026-09-25, via OSDev1: the Sanity fields
move off the settings screen onto it). A connection is saved only after `sources.py` has proved it.
...
not even carry an `aeo` section (tests/test_aeo_settings.py measures both).
"""
...
blueprint = Blueprint("aeo_machine_app", __name__)

HOME = "/aeo"
TOPICS = "/aeo/topics"
SETTINGS = "/aeo/settings"
SOURCES = "/aeo/sources"
...
shell.register_section(
    ...
    # Articles keeps the /aeo/topics address its forms, links and tests already use. Data sources
...
shell.register_section(
    ...
    items=[
        ...
        # /settings/aeo/google"). Owner-only, so a member is never shown a row they are refused at.
...
def aeo_sanity():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            log.info("aeo.sanity_refused", reason=str(e)[:120])
            return _page(SANITY, title, lede, _note("", str(e)) + _sanity_form(form)), 400
        log.info("aeo.sanity_connected", user=uid, project=project, dataset=dataset,
                 token_changed=typed)
...
def aeo_posthog():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            log.info("aeo.posthog_refused", reason=str(e)[:120])
            return _page(POSTHOG, title, lede,
                         _PH_CSS + _note("", str(e)) + _posthog_form(form)), 400
        log.info("aeo.posthog_connected", user=uid, host=host, project=project, views_30d=views)
...
# THE OLD ADDRESSES REDIRECT (owner, 2026-09-25: "Yes, please code rename"). A bookmark, a browser
# tab or a link in an old email to the machine's old address lands on the same screen under /aeo.
# 308, not 301, so a form posted to an old address stays a POST. The old prefix is spelled apart so
# no search-and-replace can ever turn this into a route that redirects to itself.
_OLD_HOME = "/" + "s" + "eo"


@blueprint.route(_OLD_HOME, methods=["GET", "POST"])
@blueprint.route(_OLD_HOME + "/<path:rest>", methods=["GET", "POST"])
def aeo_old_address(rest: str = ""):
    target = HOME + ("/" + rest.lstrip("/") if rest else "")
    qs = request.query_string.decode("utf-8", "replace")
    return redirect(target + ("?" + qs if qs else ""), code=308)
