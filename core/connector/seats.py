...
def mint(label: str, role: str) -> tuple[str, str]:
    ...
    log.info("connector.seat_minted", seat_id=seat_id, label=label, role=role)
...
def verify(credential: str) -> dict | None:
    ...
    try:
        with state.connect() as c:
            row = c.execute(
                "SELECT id, label, role, secret_hash, revoked_at FROM seats WHERE id = ?",
                (seat_id,),
            ).fetchone()
...
def all_seats(include_revoked: bool = True) -> list[dict]:
    ...
    sql = ("SELECT id, label, role, created_at, last_seen_at, revoked_at FROM seats"
           + ("" if include_revoked else " WHERE revoked_at IS NULL")
